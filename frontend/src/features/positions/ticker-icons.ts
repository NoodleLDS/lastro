...
export function getTickerIcon(ticker: string, assetType: string): string | null {
  ...
  if (TICKER_ICONS[key]) return `${import.meta.env.BASE_URL}${TICKER_ICONS[key]}`
  if (assetType === 'fii' || FII_SUFFIX.test(key))
    return `${import.meta.env.BASE_URL}branding/tickers/fii-generic.svg`
  return null
}
