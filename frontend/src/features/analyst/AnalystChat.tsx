import { BrainCircuit, Copy, RotateCcw, Sparkle, ThumbsDown, ThumbsUp } from 'lucide-react'
import { useEffect, useState } from 'react'
import { Button } from '@/components/ui/button'
import { Textarea } from '@/components/ui/textarea'
import { cn } from '@/lib/utils'
import { AnalystContextPanel } from './AnalystContextPanel'
...
export function AnalystChat() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < history.length; i += 2) {
      ...
      loaded.push({
        ...
        model: assistantMsg?.model ?? null,
        tokensPerSecond: assistantMsg?.tokens_per_second ?? null,
      })
    }
    setMessages(loaded)
  ...
  async function sendQuestion(asked: string, messageId: number) {
    const result = await stream.ask(asked, conversationId)
    if (result.conversationId !== null) setConversationId(result.conversationId)
    setMessages((prev) =>
      prev.map((message) =>
  ...
  return (
    <div className="flex h-[calc(100vh-12rem)] w-full gap-4">
      <ConversationSidebar selectedId={conversationId} onSelect={setConversationId} />
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex justify-end pb-2">
          <Button
            variant="outline"
            size="icon"
            aria-label="Contexto do analista"
        ...
        <div className="flex flex-1 flex-col gap-8 overflow-y-auto pb-4">
          ...
          {messages.length === 0 && (
            <div className="flex flex-1 items-center justify-center text-muted-foreground">
              <p className="text-sm">
                Pergunte sobre sua carteira, um ativo, ou peça uma simulação.
              </p>
            </div>
          )}
        </div>
      ...
      <AnalystContextPanel open={contextPanelOpen} onClose={() => setContextPanelOpen(false)} />
    </div>
  )
}
